...
useEffect(() => {
	let interval;
	let lastSoundPlayTime = 0; // Track when sound was last played
	...
	const fetchOrders = async () => {
		try {
			...
			// If there are new notifications and enough time has passed since last sound, play sound
			const now = Date.now();
			if (hasNewNotification && (now - lastSoundPlayTime) > SOUND_COOLDOWN_MS) {
...
}, [roomNumber, removedOrdersMap, shouldShowOrder, seenOrderStatuses]); // Removed orderStatusMap and visibleOrders
