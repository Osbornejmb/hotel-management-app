...
const EmployeeManagementSection = () => {
  ...
  // Check if cardId already exists for a different employee
  const checkCardIdExists = async (cardId, excludeEmployeeId = null) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/employee`);
      const data = await parseResponse(res);
      const allEmployees = Array.isArray(data) ? data : [];
      
      // Check if any employee has this cardId (excluding the current employee if editing)
      const duplicate = allEmployees.find(emp => 
        emp.cardId === cardId && 
        (excludeEmployeeId ? emp._id !== excludeEmployeeId : true)
      );
      
      console.log('Card ID check for', cardId, '- exists:', !!duplicate);
      return !!duplicate; // Returns true if duplicate found
    } catch (err) {
  ...
  // Find the first available cardId that hasn't been assigned to any employee
  const getAvailableCardId = async () => {
    ...
    try {
      ...
      const data = await parseResponse(res);
      const allEmployees = Array.isArray(data) ? data : [];
      const usedCardIds = new Set(allEmployees.map(emp => emp.cardId).filter(Boolean));
      
      console.log('Used card IDs:', Array.from(usedCardIds));
      
      // Find the first unused cardId
      for (const cardId of availableCardIds) {
        if (!usedCardIds.has(cardId)) {
          console.log('Available card ID found:', cardId);
          return cardId;
        }
      }
      
      // If all are used, return the first one (shouldn't happen in normal use)
      console.warn('All card IDs are in use, returning first one');
      return availableCardIds[0];
    } catch (err) {
      console.error('Error getting available card ID:', err);
      return availableCardIds[0];
    }
  };
 ...
 const handleSubmit = async (e) => {
  ...
  const emailUsed = form.email || `${form.username || form.name || form.id}@example.com`;
  
  // Get an available cardId if the current one is in use
  let cardIdToUse = form.idCard || '';
  let cardIdWasAutoAssigned = false;
  if (cardIdToUse) {
    const cardIdExists = await checkCardIdExists(cardIdToUse);
    if (cardIdExists) {
      // Auto-assign an available card ID
      const originalCardId = cardIdToUse;
      cardIdToUse = await getAvailableCardId();
      cardIdWasAutoAssigned = true;
      console.log(`Card ID ${originalCardId} was in use, auto-assigned to: ${cardIdToUse}`);
    }
  }
  ...
  try {
    ...
    // Build success message with card assignment info
    let successMessage = `Employee ${payload.name} added successfully! Password: ${autoGeneratedPassword}`;
    if (cardIdToUse) {
      if (cardIdWasAutoAssigned) {
        successMessage += ` Card auto-assigned: ${cardIdToUse} (requested ID was already in use)`;
      } else {
        successMessage += ` Card assigned: ${cardIdToUse}`;
      }
    }
