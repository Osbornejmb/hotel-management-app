...
const EmployeeManagementSection = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    let successMessage = `Employee ${payload.name} added successfully! `;
    
    if (data.emailSent !== undefined) {
      // If backend returns emailSent field
      if (data.emailSent) {
        successMessage += `✅ Credentials sent to ${payload.email}`;
      } else {
        successMessage += `📧 Email failed: ${data.emailError || 'Unknown error'}. `;
      }
    } else if (data.success !== undefined) {
      // If backend returns success field (from sendEmployeeCredentials)
      if (data.success) {
        successMessage += `✅ Credentials sent to ${payload.email}`;
      } else {
        successMessage += `📧 Email failed: ${data.message || 'Unknown error'}. `;
      }
    } else {
      // Fallback - assume email was attempted but status unknown
